...
// Extracted social media icons for better maintainability
const SOCIAL_ICONS = {
  youtube: (
...
const SocialMediaLink = memo<{ platform: keyof typeof SOCIAL_LINKS; href: string }>(
  ({ platform, href }) => (
    <a
      href={href}
...
const NavigationLink = memo<{ href: string; children: React.ReactNode }>(
  ({ href, children }) => (
    <a
      href={href}
      className="text-footer-link hover:text-gray-200 transition-colors"
    ...
    <footer className="w-full bg-primary py-8">
      <Container>
        ...
        <div className="flex flex-wrap gap-6 text-footer-link justify-center my-2">
          <NavigationLink href={NAVIGATION_LINKS.privacyPolicy}>
            Privacy Policy
          </NavigationLink>
          <NavigationLink href={NAVIGATION_LINKS.termsAndConditions}>
            Terms of Service
          </NavigationLink>
          <NavigationLink href={`mailto:${CONTACT_INFO.email}`}>
            {CONTACT_INFO.email}
          </NavigationLink>
        </div>
